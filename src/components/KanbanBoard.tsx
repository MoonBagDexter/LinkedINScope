...
export function KanbanBoard() {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      const job = newJobs[Math.floor(Math.random() * newJobs.length)];
  ...
  const handleQuickApply = async (job: JobWithClickCount) => {
    ...
    addActivity({ wallet: walletAddress, action: 'started application', jobTitle: job.job_title, employer: job.employer_name });
  ...
  const handleSubmitForm = async (data: { name: string; age: string; availability: string; girthSize: string }) => {
    ...
    addActivity({ wallet: walletAddress, action: 'completed application', jobTitle: formJob.job_title, employer: formJob.employer_name });
    setFormJob(null);
  ...
  const handleShareX = (job: JobWithClickCount) => {
    if (walletAddress) addActivity({ wallet: walletAddress, action: 'shared on X', jobTitle: job.job_title, employer: job.employer_name });
    ...
    <div>
      <div className="flex flex-col sm:flex-row gap-3 mb-6">
        ...
        <div className="flex gap-2">
          {filters.map(f => (
            <button key={f.key}
              onClick={() => { setFilterType(f.key); setPages({ new: 0, trending: 0, graduated: 0 }); }}
              className={`px-4 py-2 rounded-xl text-sm font-bold transition-all ${
                filterType === f.key ? 'bg-primary text-white shadow-md' : 'bg-cream-dark border border-cream-border text-text-secondary hover:bg-cream hover:border-primary/30'
              }`}>
              {f.label}
            </button>
          ))}
          ...
          return (
            <div key={lane} className={`${lane === activeMobileLane ? 'block' : 'hidden'} md:block`}>
              <div className="flex items-center justify-between mb-4 px-2">
                <h2 className="text-sm font-bold tracking-wider text-text-primary">{title}</h2>
                ...
                {paged.length === 0 ? (
                  <div className="text-center py-10 text-text-muted border-2 border-dashed border-cream-border bg-cream-dark rounded-xl">
                    <p className="font-medium">
                      {lane === 'trending' ? 'Hit Quick Apply to get started' : lane === 'graduated' ? 'Complete an application to see it here' : searchQuery ? 'No jobs match your search' : 'No jobs yet'}
                    </p>
                  </div>
                ) : (
                  paged.map(job => (
                    <JobCard key={job.job_id} job={job} appStatus={getStatus(job.job_id)}
                      onQuickApply={() => handleQuickApply(job)} onOpenApplyForm={() => handleOpenForm(job)}
                      onShareX={() => handleShareX(job)} isDripping={drippingId === job.job_id} />
                  ))
                )}
              </div>
              {tp > 1 && (
                <div className="flex items-center justify-center gap-3 mt-4">
                  <button onClick={() => setPages(p => ({ ...p, [lane]: Math.max(0, p[lane] - 1) }))} disabled={pages[lane] === 0}
                    className="px-4 py-1.5 text-sm rounded-lg border border-cream-border hover:bg-cream-dark disabled:opacity-30 transition-all font-bold">Prev</button>
                  <span className="text-sm text-text-muted font-medium">{pages[lane] + 1} / {tp}</span>
                  <button onClick={() => setPages(p => ({ ...p, [lane]: Math.min(tp - 1, p[lane] + 1) }))} disabled={pages[lane] >= tp - 1}
                    className="px-4 py-1.5 text-sm rounded-lg border border-cream-border hover:bg-cream-dark disabled:opacity-30 transition-all font-bold">Next</button>
                </div>
              )}
            </div>
          );
      ...
      {formJob && <ApplyForm jobTitle={formJob.job_title} employerName={formJob.employer_name} onSubmit={handleSubmitForm} onClose={() => setFormJob(null)} />}
    </div>
  );
}
